...
# 加载训练用的特征和标签
train_features = np.load('train_8k_fbank_features.npy')
print ("train_8k_fbank_features=",train_features)
# train_features = np.load('train_fbank_features.npy')
train_labels = np.load('train_8k_labels.npy')
...
def get_train_set():
    ...
    np.random.seed(10)
    shuffle_indices = np.random.permutation(np.arange(len(tr_data))) #排列组合数据
...
if __name__ == "__main__":
    ...
    # predict(wav_files)
    # end_start = time.time()
    # print("runtime:", end_start-time_start, "s")
