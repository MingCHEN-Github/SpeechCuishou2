...
def extract_labels(wav_files):
    ...
    #转换成one-hot独热编码形式
    zero_array = np.zeros(16,dtype=int)
    one_hot_array = []
    for i in labels_set:
        zero_array[i]=1
        temp =copy.deepcopy(zero_array) #深拷贝，创建内存空间存储，而不是直接赋值传引用
        #print(temp)
        one_hot_array.append(list(temp))
        #one_hot_array =
        #print("---->",one_hot_array)
        zero_array[i]=0
    #print(one_hot_array)
    labels_set = one_hot_array
    return np.array(labels_set,  dtype=np.int)
#
if __name__ == '__main__':
    wav_files = get_wav_files("audio8k", "1,2,3,4,5")
    # train_8k_features = extract_features(wav_files)
    train_8k_fbank_features = extract_fbank_features(wav_files)
    train_labels = extract_labels(wav_files)
    print(train_labels)
    np.save('train_8k_fbank_features.npy', train_8k_fbank_features)
    #np.save('train_fbank_features.npy', train_fbank_features)
